...
open class JasonComponent {
    companion object {
        ...
        fun build(
            ...
            val corner_radius = 0
            view.tag = component
            val style: JSONObject = JasonHelper.style(component, root_context!!)
            return try {
                if (parent == null) {
                    // Layer type
                    ...
                    if (style.has("height")) {
                        try {
                            ...
                            if (style.has("ratio")) {
                                val ratio: Float = JasonHelper.ratio(style.getString("ratio"))
                                width = height * ratio
                            }
                        } catch (e: Exception) {
                            Log.d(
                ...
                if (style.has("background")) {
                    val color: Int = JasonHelper.parse_color(style.getString("background"))
                    bgcolor = color
                ...
                // заполнение
                var padding_left =
                    JasonHelper.pixels(root_context, "0", "horizontal").toInt()
                var padding_right =
                    JasonHelper.pixels(root_context, "0", "horizontal").toInt()
                var padding_top =
                    JasonHelper.pixels(root_context, "0", "horizontal").toInt()
                ...
                if (style.has("corner_radius")) {
                    val corner: Float = JasonHelper.pixels(
                        root_context,
                        style.getString("corner_radius"),
                        "horizontal"
                    )
                    var color: Int =
                        ContextCompat.getColor(root_context,  android.R.color.transparent)
        ...
        fun addListener(view: View, root_context: Context) {
            val clickListener =
                View.OnClickListener { v ->
                    ...
                    try {
                        if (component.has("action")) {
                            ...
                            val href: JSONObject = component.getJSONObject("href")
