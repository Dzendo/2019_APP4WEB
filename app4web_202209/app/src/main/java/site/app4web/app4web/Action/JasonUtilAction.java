...
public class JasonUtilAction {
    private int counter; // general purpose counter; счетчик общего назначения;
    private Intent callback_intent;  // general purpose intent; намерение общего назначения;
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Toast toast = Toast.makeText(context, options.get("text").toString(), duration);
                    toast.show();
                } catch (Exception e){
                    Log.d("Warning", e.getStackTrace()[0].getMethodName() + " : " + e.toString());
                }
                ...
                try {
                    ...
                    if (action.has("options")) {
                        ...
                        if(options.has("form"))
                        {
                            ...
                            final EditText focousedTextField = textFields.get(0);
