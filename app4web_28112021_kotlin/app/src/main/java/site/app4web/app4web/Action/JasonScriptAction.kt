...
class JasonScriptAction {
    fun include(action: JSONObject, data: JSONObject?, event: JSONObject?, context: Context) {
        try {
            val options = action.getJSONObject("options")
            if (options.has("items")) {
                val items = options.getJSONArray("items")
                val refs = JSONObject()
                val client = (context.applicationContext as Launcher).getHttpClient(0)
                ...
                if (urlItems.length() > 0) {
                    ...
                    val taskExecutor = Executors.newFixedThreadPool(urlItems.length())
