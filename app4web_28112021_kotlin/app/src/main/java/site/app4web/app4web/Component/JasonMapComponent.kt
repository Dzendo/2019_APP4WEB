...
object JasonMapComponent : JasonComponent() {
        ...
        if (view == null) {
            try {
                ...
                if (component.has("region")) {
                    latlng = JasonMapComponent.getCoordinates(component.getJSONObject("region"))
                }
                ...
                (context as JasonViewActivity).addListViewOnItemTouchListener(JasonMapComponent.touchListener)
    ...
    internal class MapReadyHandler(
        ...
        private fun getZoomForMetersWide(meters: Double, width: Double, lat: Double): Double {
            ...
            val arg = JasonMapComponent.EQUATOR_LENGTH * width * latAdjust / (meters * 256.0)
            ...
            try {
                ...
                if (component.has("pins")) {
                    ...
                    for (i in 0 until pins.length()) {
                        ...
                        options.position(JasonMapComponent.getCoordinates(pin))
                        if (pin.has("title")) {
                        ...
                        if (pinJSONObject!!.has(ACTION_PROP)) {
                            ...
                            pinData.put(
                                MAP_PIN_COORD_PROP, String.format(
                                    JasonGeoAction.COORDS_STRING_FORMAT,
                                    pos.latitude,
                                    pos.longitude
                                )
                            )
