...
class JasonOauthAction {
    ...
    fun auth(action: JSONObject, data: JSONObject, event: JSONObject, context: Context) {
        try {
            ...
            if (options.has("version") && options.getString("version") == "1") {
                    ...
                    object : AsyncTask<String?, Void?, Void?>() {
                        protected override fun doInBackground(vararg params: String?): Void? {
                            try {
                                ...
                                val auth_url = oauthService.getAuthorizationUrl(request_token)
                ...
                if (authorize_options_data.has("grant_type") && authorize_options_data.getString("grant_type") == "password") {
                    ...
                    if (!authorize_options.has("scheme") || authorize_options.getString("scheme").length == 0 || !authorize_options.has(
                            "host"
                        ) || authorize_options.getString("host").length == 0 || !authorize_options.has(
                            "path"
                        ) || authorize_options.getString("path").length == 0 || !authorize_options_data.has(
                            "username"
                        ) || authorize_options_data.getString("username").length == 0 || !authorize_options_data.has(
                            "password"
                        ) || authorize_options_data.getString("password").length == 0
                    ) {
                        JasonHelper.next("error", action, data, event, context)
                    } else {
                        ...
                        val paramKeys: Iterator<*> = authorize_options_data.keys()
                        ...
                        object : AsyncTask<String?, Void?, Void?>() {
                            protected override fun doInBackground(vararg params: String?): Void? {
                                try {
                                    ...
                                    val access_token = oauthService.getAccessTokenPasswordGrant(
                                        username,
                                        password
                                    ).accessToken
                                    val preferences =
                        ...
                        if (authorize_options.has("data")) {
                            ...
                            if (authorize_options.length() == 0) {
                                ...
                                if (!authorize_options.has("scheme") || authorize_options.getString(
                                    ...
                                    val intent = Intent(Intent.ACTION_VIEW)
                                    intent.data =
                                        Uri.parse(oauthService.getAuthorizationUrl(additionalParams))
                                    val callback = JSONObject()
                                    callback.put("class", "JasonOauthAction")
                                    callback.put("method", "oauth_callback")
    ...
    fun access_token(action: JSONObject, data: JSONObject?, event: JSONObject, context: Context) {
        try {
            val options = action.getJSONObject("options")
            val client_id = options.getJSONObject("access").getString("client_id")
            val sharedPreferences = context.getSharedPreferences("oauth", Context.MODE_PRIVATE)
    ...
    fun oauth_callback(intent: Intent, intent_options: JSONObject) {
        try {
            ...
            val context = intent_options["context"] as Context
            val options = action.getJSONObject("options")
            val uri = intent.data
            if (options.has("version") && options.getString("version") == "1") {
                //OAuth 1
                    ...
                    object : AsyncTask<String?, Void?, Void?>() {
                        protected override fun doInBackground(vararg params: String?): Void? {
                            try {
                                ...
                                preferences.edit().putString(client_id, access_token.token).apply()
                                preferences.edit().putString(
                ...
                if (access_token != null && access_token.length > 0) {
                    val client_id = authorize_options.getString("client_id")
                    val preferences = context.getSharedPreferences("oauth", Context.MODE_PRIVATE)
                    preferences.edit().putString(client_id, access_token).apply()
                    val result = JSONObject()
                    result.put("token", access_token)
                    JasonHelper.next("success", action, result, event, context)
                        ...
                        protected override fun doInBackground(vararg params: Void?): Void? {
                            try {
                                ...
                                if (response.code == 200) {
                                    ...
                                    JasonHelper.next("error", action, response.body, event, context)
                                }
                            } catch (e: Exception) {
                                handleError(e, action, event, context)
                            }
