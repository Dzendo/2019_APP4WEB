...
object JasonHelper {
    fun style(component: JSONObject?, root_context: Context): JSONObject {
        ...
        try {
            if (component!!.has("class")) {
                ...
                val style_classes =
                    style_class_string.split("\\s+".toRegex()).toTypedArray()
                for (i in style_classes.indices) {
                    val astyle =
                        (root_context as JasonViewActivity).model!!.jason!!.getJSONObject("\$jason")
                            .getJSONObject("head").getJSONObject("styles")
                            .getJSONObject(style_classes[i])
    ...
    fun pixels(
        ...
        val percent_pixels =
            Pattern.compile(regex_percent_and_pixels)
