            ...
            if (action is JSONArray) {
                ...
                JasonParser.getInstance(this)!!.setParserListener(object : JasonParserListener {
                    override fun onFinished(reduced_action: JSONObject?) {
                        final_call(reduced_action, data, ev, context)
                    }
                })
        ...
        try {
            ...
            if (type.startsWith("$") || type.startsWith("@")) {
                ...
                if (tokens.size == 1) {
                    ...
                    try {
                        val `is` =
                            assets.open("file/$$className.json")
...
        4. Когда подпрограмма вызывает `$ return.error`, продолжайте выполнение из действия` error`, используя возвращаемое значение из подпрограммы.
        ...
        try {
            if (action.has("options")) {
                ...
                if (urlSet.size > 0) {
                    val refs = JSONObject()
                    val latch =
                        CountDownLatch(urlSet.size)
                    val taskExecutor =
                        Executors.newFixedThreadPool(urlSet.size)
                    for (key in urlSet) {
                        ...
                        if (bg is String) {
                            ...
                            if (background.matches("(file|http[s]?):\\/\\/.*".toRegex())) {
                                if (backgroundImageView == null) {
                                    backgroundImageView =
                                        ImageView(this@JasonViewActivity)
                                }
    ...
    private fun setup_header(header: JSONObject) {
        try {
            val backgroundColor =
                header.getJSONObject("style").getString("background")
