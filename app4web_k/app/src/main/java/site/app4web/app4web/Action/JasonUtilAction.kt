...
class JasonUtilAction {
    private var counter:Int = 0 // general purpose counter; счетчик общего назначения; = 0
    ...
    fun toast(
        ...
        Handler(Looper.getMainLooper()).post {
            try {
                ...
                val toast =
                    Toast.makeText(context, options["text"].toString(), duration)
                toast.show()
            } catch (e: Exception) {
                Log.d(
                    "Warning",
    ...
    fun addressbook(
        ...
        Thread(Runnable { getContacts(action, data, event, context) }).start()
    }
                    ...
                    for (i in 0 until l) {
                        ...
                        if (item.has("type")) {
                            ...
                            if (type.equals("text", ignoreCase = true)) {
                                ...
                                if (item.has("url")) {
                                    val helper =
                                        JasonImageHelper(item.getString("url"), context)
                                ...
                                if (item.has("file_url")) {
                                    val uri =
                                        Uri.parse(item.getString("file_url"))
                                    callback_intent!!.putExtra(Intent.EXTRA_STREAM, uri)
                                    // override with image type if one of the items is an image
