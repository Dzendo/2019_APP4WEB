...
object JasonImageComponent {
    private fun prepare(
        ...
        return try {
            ...
            val uri_for_session =
                URI(component.getString("url").toLowerCase())
    ...
    fun resolve_url(component: JSONObject, context: Context): Any {
        return try {
            ...
            if (url.contains("file://")) {
                ...
                val builder = prepare(component, context)
    ...
    private fun normal(
        ...
        val new_url = resolve_url(component, context)
        if (new_url.javaClass.toString().equals(
                "string",
                ignoreCase = true
            ) && (new_url as String).startsWith("data:image")
        ) {
            val n = new_url
            val base64: String
