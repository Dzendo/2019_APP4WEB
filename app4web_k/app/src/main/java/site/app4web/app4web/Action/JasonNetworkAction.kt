...
class JasonNetworkAction {
        ...
        try {
            ...
            if (options.has("url")) {
                ...
                if (session != null && session.has("header")) {
                    val keys: Iterator<*> =
                        session.getJSONObject("header").keys()
                    while (keys.hasNext()) {
                ...
                if (method.equals("get", ignoreCase = true)) {
                    ...
                    if (session != null && session.has("body")) {
                        val keys: Iterator<*> =
                            session.getJSONObject("body").keys()
                        while (keys.hasNext()) {
                            val key = keys.next() as String
                    ...
                    if (options.has("header") && options.getJSONObject("header")
                        ...
                        val content_type =
                            options.getJSONObject("header").getString("content_type")
                        ...
                        if (session != null && session.has("body")) {
                            ...
                            while (keys.hasNext()) {
                                val key = keys.next() as String
                                val `val` =
                                    session.getJSONObject("body").getString(key)
                                bodyBuilder.add(key, `val`)
                            }
                        }
                ...
                client = if (options.has("timeout")) {
                    ...
                    if (timeout is Long) {
                        ...
                        val timeout_int: Long = timeout.toLong ()
    ...
    fun upload(
        ...
        try {
            val options = action.getJSONObject("options")
            if (options.has("data")) {
                var stack: JSONObject? = JSONObject()
