...
public class JasonHelper {
    ...
    public static Object read_json(String fn, Context context) { //  throws IOException {
        ...
        InputStream is;
        File sdFile;
        String filename = fn.replace("file://read//", "");
        try {
  ...
  // вызывается из read_json расшифровывает строку в json object DD
    private static Object str_to_json(String jr) {
        Object ret;
        try {
            if (jr.trim().startsWith("[")) {        // array  массив
                ret = new JSONArray(jr);
            } else if (jr.trim().startsWith("{")) { // object объект
    ...
    public static Object read_json_old(String fn, Context context) {// throws IOException {
        ...
        InputStream is;
        File sdFile;
        String filename = fn.replace("file://", "");

        try {
            if (filename.trim().startsWith("#")) {           // из памяти приложения
                is = context.openFileInput(filename);
            } else if (filename.trim().startsWith("/")) {    // из SD памяти
                sdFile = new File(filename);
                is = new FileInputStream(sdFile);
...
*/
    ...
    public static void dispatchIntent(String name, JSONObject action, JSONObject data, JSONObject event, Context context, Intent intent, JSONObject handler) {
        ...
        try {
            ...
            ((Launcher) context.getApplicationContext()).once(name, handler);
        } catch (Exception e) {
            Timber.w(e);
        }
