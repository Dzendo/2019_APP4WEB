package site.app4web.app4web.Service.websocket //import
 ...
 *
 */
class JasonWebsocketService(launcherParam: Launcher) {
    companion object {  // перенес выше из inner class
        private const val NORMAL_CLOSURE_STATUS = 1000
    }
    private inner class JasonWebSocketListener : WebSocketListener() {
        override fun onOpen(webSocket: WebSocket, response: Response) {
            val context: JasonViewActivity =
    ...
    fun open(action: JSONObject) {
        try {
            val options: JSONObject = action.getJSONObject("options")
            val url: String = options.getString("url")
            val client: OkHttpClient = launcher.getHttpClient(0)
    ...
    fun send(action: JSONObject) {
        try {
            val options: JSONObject = action.getJSONObject("options")
            val text: String = options.getString("message")
            ws?.send(text)
        } catch (e: Exception) {
            Log.d(
                "Warning",
