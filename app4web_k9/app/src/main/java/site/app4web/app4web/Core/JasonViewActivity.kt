...
open class JasonViewActivity : AppCompatActivity(),
            ...
            if (head.has("agents")) {
                ...
                while (iterator.hasNext()) {
                    ...
                    runOnUiThread {
                        try {
                            val agentService =
                                (applicationContext as Launcher).services?.get("JasonAgentService") as JasonAgentService
    ...
    private fun onSwitchTab(newUrl: String, newParams: String, intent: Intent) {
        ...
        try {
            ...
            val m =
                (applicationContext as Launcher).getTabModel(newUrl + newParams)
            if (m == null) {
                // refresh
                removeListViewOnItemTouchListeners()
                    ...
                    JasonParser.Companion.getInstance(this)
                        ?.setParserListener(object : JasonParserListener {
                            override fun onFinished(parsed_options: JSONObject?) {
                                try {
                                    val action_with_parsed_options =
    ...
    private fun exec(
        ...
        try {
            ...
            if (type.startsWith("$") || type.startsWith("@")) {
                val tokens = type.split("\\.".toRegex()).toTypedArray()
            ...
            try {
                ...
                Log.d(
                    "Warning",
                    e2.stackTrace[0].methodName + " : " + e2.toString()
                )
            }
        }
    }
...
        4. Когда подпрограмма вызывает `$ return.error`, продолжайте выполнение из действия` error`, используя возвращаемое значение из подпрограммы.
    ...
    private fun setup_body(body: JSONObject?) {
        ...
        runOnUiThread {
            try {
                ...
                window.decorView
                    .setBackgroundColor(JasonHelper.parse_color("rgb(255,255,255)"))
                var bg: Any? = null
                    ...
                    if (needs_redraw) {
                        if (bg is String) {
                            ...
                            if (background.matches("(file|http[s]?):\\/\\/.*".toRegex())) {
                                ...
                                var cacheStrategy = DiskCacheStrategy.DATA
                                ...
                                if (background.equals("camera", ignoreCase = true)) {
                                    ...
                                    if (cameraManager == null) {
                                        cameraManager =
                                            JasonVisionService(this@JasonViewActivity)
                                    ...
                                    window.decorView
                                        .setBackgroundColor(JasonHelper.parse_color(background))
                                }
                            }
                        } else {
                            ...
                            if (type.equals("html", ignoreCase = true)) {
                                ...
                                if (background.has("options")) {
                                    val options =
                                        background.getJSONObject("options")
                ...
                if (body.has("sections")) {
                    ...
                    if (!border.equals("none", ignoreCase = true)) {
                        ...
                        listView!!.removeItemDecoration(divider!!)
                        divider =
                            HorizontalDividerItemDecoration.Builder(this@JasonViewActivity)
                                .color(color)
                                .showLastDivider()
                                .positionInsideItem(true)
                                .build()
                        val _divider = divider!!  // AS
                        listView!!.addItemDecoration(_divider)
                    }
                } else {
                    setup_sections(null)
                }
                ...
                if (model!!.jason != null && model!!.jason!!.has("\$jason") && model!!.jason!!.getJSONObject(
                    ...
                    if (head.has("actions") && head.getJSONObject("actions").has("\$pull")) {
                        ...
                        swipeLayout!!.setOnRefreshListener {
                            try {
                                val action =
                                    head.getJSONObject("actions").getJSONObject("\$pull")
                                call(
                                    action.toString(),
                                    JSONObject().toString(),
                                    "{}",
    ...
    private fun setup_sections(sections: JSONArray?) {
        ...
        if (adapter == null || adapter!!.items?.size ?: 0  == 0) {
            // Create adapter passing in the sample user data
    ...
    private fun setup_input(input: JSONObject) {
        ...
        val height =
            JasonHelper.pixels(this@JasonViewActivity, "60", "vertical").toInt()
        val spacing =
            JasonHelper.pixels(this@JasonViewActivity, "5", "vertical").toInt()
        val outer_padding =
            JasonHelper.pixels(this@JasonViewActivity, "10", "vertical").toInt()
        ...
        try {
            ...
            val layout_params =
                footer_input_textfield!!.layoutParams as LinearLayout.LayoutParams
        ...
        try {
            ...
            if (tabs.has("style")) {
                ...
                if (style.has("color:disabled")) {
                    val disabled_color =
                        JasonHelper.parse_color(style.getString("color:disabled"))
                    bottomNavigation!!.inactiveColor = disabled_color
                }
                if (style.has("background")) {
                    val background =
                        JasonHelper.parse_color(style.getString("background"))
                            ...
                            .into(object : SimpleTarget<Bitmap?>(100, 100) {
                                override fun onResourceReady(
                                    ...
                                    val drawable: Drawable =
                                        BitmapDrawable(resources, resource)
                                    val item =
                                        AHBottomNavigationItem(text, drawable)
            ...
            if (model!!.rendered != null) {
                ...
                if (header.has("search")) {
                    ...
                    c = if (search.has("style") && search.getJSONObject("style").has("color")) {
                        JasonHelper.parse_color(
                            search.getJSONObject("style").getString("color")
                        )
                    ...
                    searchView!!.setOnQueryTextListener(object :
                        ...
                        override fun onQueryTextChange(s: String): Boolean {
                            return if (search.has("action")) {
                                ...
                                if (listView != null) {
                                    val adapter =
                                        listView!!.adapter as ItemAdapter?
                                    adapter!!.filter(s)
                                }
                                true
                            }
                ...
                if (header.has("menu")) {
                    ...
                    if (json.has("image")) {
                        val padding =
                            JasonHelper.pixels(this, "15", "vertical").toInt()
                    ...
                    if (json.has("badge")) {
                        ...
                        var left =
                            JasonHelper.pixels(this, 30.toString(), "horizontal").toInt()
                        var top =
                            JasonHelper.pixels(this, (-3).toString(), "vertical").toInt()
                        ...
                        try {
                            ...
                            if (header.has("menu")) {
                                if (header.getJSONObject("menu").has("action")) {
                                    ...
                                    val action = JSONObject().put("type", "\$href")
                                        .put(
                                            "options",
