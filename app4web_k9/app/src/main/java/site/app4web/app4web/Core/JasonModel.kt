...
class JasonModel(var url: String?, intent: Intent?, var view: JasonViewActivity) {
    ...
    private fun resolve_and_build(res: String) {
        try {
            ...
            if (matcher.find()) {
                ...
                val local_regex = "\"([+@])\"[ ]*:[ ]*\"(([^\"@]+)(@))?([^\"]+)\""
                val local_require_pattern =
                    Pattern.compile(local_regex)
    ...
    private fun resolve_reference() {
        ...
        try {
            ...
            val remote_pattern_with_path_str =
                "\"([+@])\"[ ]*:[ ]*\"(([^$\"@]+)(@))([^\"]+)\""
            val remote_pattern_with_path =
                Pattern.compile(remote_pattern_with_path_str)
            val remote_with_path_matcher =
                remote_pattern_with_path.matcher(str_jason)
            str_jason =
            ...
            val remote_pattern_without_path =
                Pattern.compile(remote_pattern_without_path_str)
            val remote_without_path_matcher =
                remote_pattern_without_path.matcher(str_jason)
            str_jason =
            ...
            JasonParser.Companion.getInstance(view)
                ?.setParserListener(object : JasonParserListener  {  // resolved_jason
    ...
    private fun resolve_local_reference() {
        ...
        try {
            ...
            val local_pattern =
                Pattern.compile(local_pattern_str)
        ...
        if (name.equals("jason", ignoreCase = true)) {
            ...
            try {
                ...
                state = if (jason!!.getJSONObject("\$jason")
                        .has("head") && jason!!.getJSONObject("\$jason")
                        .getJSONObject("head").has("data")
                ) {
                ...
                state!!.put("\$global", (view.applicationContext as Launcher).getGlobal())
                state!!.put("\$env", (view.applicationContext as Launcher).getEnv())
