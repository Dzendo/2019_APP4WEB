...
public class JasonViewActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback{
    ...
    private SwipeRefreshLayout swipeLayout;  // «резинка от трусов»  «потяни, чтобы обновить»
...
//  3230     Конец  JasonViewActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(rootLayout == null) {
            ...
            rootLayout.setLayoutParams(rlp);        // будет занимать все доступное ему пространство.
            rootLayout.setFitsSystemWindows(true);  //  оставить пространство для системных окон
        }
            ...
            if (type.startsWith("$") || type.startsWith("@")){
                ...
                if(tokens.length == 1){
                    ...
                    if(resolved_classname != null) {
                        fileName = "site.app4web.app4web.Action." + resolved_classname;
                    } else {
                        fileName = "site.app4web.app4web.Action.Jason" + className.toUpperCase().charAt(0) + className.substring(1) + "Action";
                    }
...
        4. Когда подпрограмма вызывает `$ return.error`, продолжайте выполнение из действия` error`, используя возвращаемое значение из подпрограммы.
        ...
        try{
            if(action.has("options")){
                ...
                // 3. Если `options` не существует, переслать данные из предыдущего действия
                else {
    ...
    private void setup_input(JSONObject input){
        ...
        try {
            ...
            View footer_input_textfield = JasonComponentFactory.build(null, textfield, null, JasonViewActivity.this);
