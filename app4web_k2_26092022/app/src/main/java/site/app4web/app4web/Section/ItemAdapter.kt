...
class ItemAdapter(root_context: Context, context: Context, var items: ArrayList<JSONObject>) :
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        var regex = "\"(url|text)\"[ ]*:[ ]*\"([^\"]+)\""
        var signature = stringified_item.replace(regex.toRegex(), "\"jason\":\"jason\"")
    ...
    inner class ViewHolderFactory {
        ...
        private var index = 0
        fun build(prototype: ViewHolder?, json: JSONObject): ViewHolder? {
            ...
            return if (exists!!) {
                ...
                val viewHolder: ViewHolder = ItemAdapter.ViewHolder(layout)
                ...
                try {
                    ...
                    for (i in 0 until components.length()) {
                        ...
                        if (component.getString("type").equals(
                            ...
                            val child_component = buildComponent(component, item)
                            if (i > 0) {
                        ...
                        if (level == 0) {
                            ...
                            if (background.matches("(file|http[s]?):\\/\\/.*")) {
                                ...
                                Glide.with(root_context)
                                    ...
                                    .into(ViewHolderFactory.BackgroundImage(layout))
                            } else {
                                // plain background
                                // простой фон
