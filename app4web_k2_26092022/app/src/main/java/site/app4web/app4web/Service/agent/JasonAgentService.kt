...
// Инициализация
{
            ...
            if (context.agents.has(id)) {
                ...
                if (id.startsWith("\$webcontainer")) {
                    ...
                    if (agent.findViewById<View?>(42) == null) {
                        pBar = ProgressBar(context, null, android.R.attr.progressBarStyleHorizontal)
                        ...
                        pBar.progressDrawable.setColorFilter(color, PorterDuff.Mode.SRC_IN)
                        pBar.id = 42
                    } else {
                        pBar = agent.findViewById(42)
                    }
                    agent.addView(pBar)
                    agent.webChromeClient = object : WebChromeClient() {
                    ...
                    agent.webChromeClient = WebChromeClient()
                }
                            ...
                            if (view.hitTestResult.type > 0) {
                                if (options.has("action")) {
                                    ...
                                    JasonParser.Companion.getInstance(context)!!.setParserListener(
                                        JasonParserListener { reduced_action ->
    ...
    fun request(jason_request: JSONObject?, agent_request: JSONObject?, context: Context) {
        try {
            /*****************************
             *
            ...
            if (jsonrpc.has("id")) {
                ...
                if ((context as JasonViewActivity).agents.has(identifier)) {
                    // Find agent by ID
                    // Найти агента по ID
                    val agent = context.agents[identifier] as WebView
            ...
            if (options.has("id")) {
                ...
                if ((context as JasonViewActivity).agents.has(identifier)) {
                    ...
                    if (options.has("items")) {
                        ...
                        val taskExecutor = Executors.newFixedThreadPool(items.length())
